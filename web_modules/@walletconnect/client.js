import { i as isBrowser, g as getLocation, d as detectEnv, a as getLocal, s as setLocal, r as removeLocal, b as getClientMeta, c as isMobile, m as mobileLinkChoiceKey } from '../common/mobile-7fed0307.js';
...
import { s as sha3 } from '../common/sha3-ad18f687.js';
...
function convertNumberToHex(num, noPrefix) {
    const hex = cjs.removeHexPrefix(cjs.sanitizeHex(new bn(num).toString(16)));
...
function toChecksumAddress(address) {
    address = cjs.removeHexPrefix(address.toLowerCase());
    const hash = cjs.removeHexPrefix(sha3.keccak_256(convertUtf8ToBuffer(address)));
    let checksum = "";
    ...
    return cjs.addHexPrefix(checksum);
}
...
async function generateKey(length) {
    ...
    const result = convertBufferToArrayBuffer(cjs.arrayToBuffer(bytes));
    return result;
}
async function verifyHmac(payload, key) {
    const cipherText = cjs.hexToArray(payload.data);
    const iv = cjs.hexToArray(payload.iv);
    const hmac = cjs.hexToArray(payload.hmac);
    const hmacHex = cjs.arrayToHex(hmac, false);
    const unsigned = cjs.concatArrays(cipherText, iv);
    const chmac = await hmacSha256Sign(key, unsigned);
    const chmacHex = cjs.arrayToHex(chmac, false);
...
async function encrypt(data, key, providedIv) {
    const _key = cjs.bufferToArray(convertArrayBufferToBuffer(key));
    const ivArrayBuffer = providedIv || (await generateKey(128));
    const iv = cjs.bufferToArray(convertArrayBufferToBuffer(ivArrayBuffer));
    const ivHex = cjs.arrayToHex(iv, false);
    const contentString = JSON.stringify(data);
    const content = cjs.utf8ToArray(contentString);
    const cipherText = await aesCbcEncrypt(iv, _key, content);
    const cipherTextHex = cjs.arrayToHex(cipherText, false);
    const unsigned = cjs.concatArrays(cipherText, iv);
...
async function decrypt(payload, key) {
    ...
    const utf8 = cjs.arrayToUtf8(buffer);
    let data;
    try {
